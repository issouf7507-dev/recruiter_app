...
const CandidaturesAccepteesPage = () => {
  ...
  const fetchCandidatures = async () => {
    try {
      setLoading(true);
      const response = await fetch("/api/candidat/candidatures-acceptees");
      const data = await response.json();

      if (data.success) {
        setCandidatures(data.data);
        console.log("Candidatures acceptées:", data.data);
      } else {
        console.error(
          "Erreur lors du chargement des candidatures acceptées:",
          data.error
        );
      }
    } catch (error) {
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
            ...
            {selectedCandidature && (
              <div className="space-y-6">
                {/* En-tête de l'offre */}
                <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/20 p-6 rounded-lg">
                  <div className="flex items-start justify-between">
                    <div className="space-y-2">
